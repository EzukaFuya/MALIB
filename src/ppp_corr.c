...
#if 0 /* endo */
#define MAX_DIST_STEC  300.0      /* max distance for stec correction (km) */
#define MAX_DIST_TROP  300.0     /* max distance for trop correction (km) */
#else
                   ...
                   int *sta, double *dist, double gpos[][3])
{
     ...
     for (i=0;i<corr->nsta;i++) {
	/* trace(2,"endo2,%d,%lf,%lf,%lf\n",i,corr->rr[i][0],corr->rr[i][1],corr->rr[i][2]); */
                         ...
                         double *std, double *el, int *flag)
{
    ...
    for (i=0;i<n;i++) {
        ...
        if (tt>=15.0) break;
#endif
	/* trace(2,"endo,%d,%lf,%lf,%d,%lf\n",i,(double)stec[i].time.time, (double)time.time, stec[i].sat,stec[i].ion); */
                        ...
                        const double *el, double *ion, double *std)
{
     ...
     for (i=0;i<n;i++) {
         ...
         for (j=0;j<MAXSAT;j++) {
             if (ion_s[j]==0.0||el[j]<=0.0) continue;
             var=SQR(std_s[j])+SQR(dist[i]*STD_VTEC_DIST/sin(el[j]));
